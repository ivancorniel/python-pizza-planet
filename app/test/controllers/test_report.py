...
def test_get_report_data(app, ingredients, sizes, client_data, beverages):
    ...
    report_data, error = ReportController.get_report_data()

    pytest.assume(error == None)
    pytest.assume(report_data['most_requested_ingredient'])
    pytest.assume(report_data['top_3_customers'])
    pytest.assume(report_data['month_with_more_revenue'])
