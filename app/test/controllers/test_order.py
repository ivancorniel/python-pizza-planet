...
def test_create(app, ingredients, size, client_data, beverages):
    created_size, created_ingredients, created_beverage = __create_sizes_and_ingredients(ingredients, [size], beverages)
    order = __order(created_ingredients, created_beverage, created_size, client_data)
...
def test_calculate_order_price(app, ingredients, size, client_data, beverages):
    created_size, created_ingredients, created_beverages = __create_sizes_and_ingredients(ingredients, [size], beverages)
    order = __order(created_ingredients, created_beverages, created_size, client_data)
    created_order, _ = OrderController.create(order)
    pytest.assume(
        created_order['total_price'] == round(created_size['price'] + sum(ingredient['price'] for ingredient in created_ingredients) + sum(beverage['price'] for beverage in created_beverages), 2)
    ) 
